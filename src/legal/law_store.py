...
def ensure_law_db_initialized() -> int:
    import os
    # ⬇️ 1. 여기에 본인의 실제 API 키를 넣으세요
    my_api_key = "AIzaSyB4TYXQYZOeoR8UlYCFe8duXLYZ3kv5st4" 

    # 2. 시스템에 강제로 주입 (이게 있어야 다른 모듈이 키를 인식함)
    os.environ["GEMINI_API_KEY"] = my_api_key
    
    # 3. 디버깅 메시지 추가
    print(f"[DEBUG] 키 주입 완료: {my_api_key[:5]}...")

    try:
        # 이제 키 검사 로직(if not os.environ...)을 무시하고 바로 호출
        return get_law_store().initialize()
    except Exception as e:
        print(f"[clair-ai] 법령 DB 초기화 중 오류 발생: {e}")
        return 0
